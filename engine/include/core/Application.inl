...
namespace novum_engine::core
{
    ...
    {
        m_platform = std::make_unique<platform::Platform>(m_event_dispatcher);
    ...
    template <typename Derived>
    void Application<Derived>::run() noexcept
    {
        auto& self = static_cast<Derived&>(*this);
        while (m_is_running)
        {
            m_platform->pollEvents();
            m_input_system.update(m_platform->getInputContext(*m_window));
